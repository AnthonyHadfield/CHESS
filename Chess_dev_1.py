...
class ChessWorld:
    def __init__(self):
        ...
        self.white_piece_board_coords = [];
        self.black_piece_board_coords = [];
        self.potential_coords_vector = []
        self.white_choice = ();
        self.black_choice = ();
        self.potential_board_vector = []
    ...
    def board(self):
        ...
        self.Bb2 = self.frame.create_image(self.bc[7][5], anchor=NW, image=self.img_bishop2_w)
    ...
    def white_pawn(self):
        ...
        if self.i_white_index == 6:
            self.potential_coords_vector = [(self.bc[5][self.white_choice]),
                                            (self.bc[4][self.white_choice])]
            self.potential_board_vector = [(self.mp[5][self.white_choice]),
                                           (self.mp[4][self.white_choice])]
            # print('VECTOR', self.potential_board_vector)
            """If 1st move is blocked PASS"""
    ...
    def white_knight(self, i, j):
        ...
        if self.start_white_pieces[self.white_choice] == 'Ktw1' or \
             self.start_white_pieces[self.white_choice] == 'Ktw2':
            """adjustement index list"""
        ...
        for i in range(0, len(index)):
            if index[i][0] > 7:
                remove1.append(i)
        """remove horizontal group"""
        for i in range(0, len(remove1)):
            index.pop(remove1[i] - i)
            """define remove remaining indexes """
        remove2 = []
        for i in range(0, len(index)):
            if index[i][1] < 0:
                remove2.append(i)
        ...
        print('final pruning result', index)
        ...
        for x in range(0, len(index)):
            ...
            self.potential_board_vector.append(vector)
        board = self.potential_board_vector
        for x in range(0, len(index)):
            if self.potential_board_vector[x] == '   ':
                self.white_move_vector.append(self.potential_coords_vector[x])
            elif board[0] != '   ' and board[1] != '   ' and board[2] != '   ':
                pass
            elif self.potential_board_vector[x] != '   ':
                piece = list(self.potential_board_vector[x])
                if piece[1] == 'w':
                    pass
                elif piece[1] == 'b':
                    self.white_move_vector.append(self.potential_coords_vector[x])
                    #   print('White Knight', self.white_move_vector)
                    self.white_move_vector = []
                    break
            ...
            for j in range(0, 8):
                if self.mp[i][j] == self.start_black_pieces[self.black_choice]:
                    ...
                    if self.i_black_index == 1:
                        self.potential_coords_vector = [(self.bc[2][self.black_choice]),
                                                        (self.bc[3][self.black_choice])]
