...
class ChessWorld:
    def __init__(self):
        ...
        self.white_piece_board_coords = []; self.black_piece_board_coords = []
...
#        self.start_white_pieces = ['Pw1', 'Pw2', 'Pw3', 'Pw4', 'Pw5', 'Pw6', 'Pw7', 'Pw8', 'Rw1', 'Ktw1', 'Bw1', 'Qw',
 #                                  'Kw', 'Bw2', 'Ktw2', 'Rw2']
    ...
    def white_piece_vector(self):
        ...
        print('choice', choice)
        #if self.start_white_pieces[choice] == 'Pw1':
        #print('PW1')
        #self.white_index_x = choice
        for i in range(0, 8):
            for j in range(0, 8):
                if self.bp[i][j] == self.start_white_pieces[choice]:
                    ...
                    if self.i_white_index == 6:
                        self.potential_coords_vector = [(self.bc[5][choice]), (self.bc[4][choice])]
                        self.potential_board_vector = [(self.bp[5][choice]), (self.bp[4][choice])]
    ...
    def black_piece_vector(self):
        ...
        for i in range(0, 8):
            for j in range(0, 8):
                if self.bp[i][j] == self.start_black_pieces[choice]:
                    ...
                    if self.i_black_index == 1:
                        self.potential_coords_vector = [(self.bc[2][choice]), (self.bc[3][choice])]
                        self.potential_board_vector = [(self.bp[2][choice]), (self.bp[3][choice])]
                        if self.potential_board_vector[0] == '   ':
    ...
    def move_random_piece(self):
        ...
        self.board()
        for i in range(1, 2):
            selected_white_move_vector = self.white_move_vector
            print('selected white vector', selected_white_move_vector)
            """select random move"""
            ...
            for m in range(0, 8):
                for n in range(0, 8):
                    if self.bc[m][n] == random_piece_board_coords:
                        ...
                        np.save('C:/Users/user/PycharmProjects/CHESS/Move_piece', self.mp)
                        print(self.mp)
            selected_black_move_vector = self.black_move_vector
            #selected_black_move_vector = self.black_move_vector

            #"""select random move"""
            move = np.random.randint(0, len(selected_black_move_vector))
            print('len', len(selected_black_move_vector))
            print('move', move)
            #"""MOVE piece"""
            random_piece_board_coords = self.black_move_vector[move]
            selection = random_piece_board_coords
            print('piece coords', self.black_piece_board_coords)
